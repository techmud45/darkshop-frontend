...
<script setup>
  ...
  const canSubmit = computed(() => {
    return newTicket.value.subject.trim() !== '' && newTicket.value.message.trim() !== '';
  });
  ...
  const submitTicket = async () => {
    try {
      const response = await axios.post('/support/tickets', newTicket.value);
  ...
  const sendReply = async () => {
    ...
    try {
      const response = await axios.post(`/support/tickets/${selectedTicket.value.id}/replies`, {
        message: replyMessage.value,
      });
      selectedTicket.value.replies.push(response.data);
      replyMessage.value = '';
    } catch (error) {
